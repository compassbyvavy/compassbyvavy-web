...
describe("factFingerprint", () => {
  ...
  it("includes knownGaps in the fingerprint (absence is a fact)", () => {
    const withoutGaps = record({
      recordType: "program",
      sourceIdentity: "ckp:program",
      normalizedFields: {
        name: "Summer Camp",
    ...
    const withGaps = record({
      ...
      normalizedFields: {
        ...
        marketingAgeMin: 4,
        marketingAgeMax: 12,
        knownGaps: ["own_counsellor_background_screening_policy_not_stated"],
      },
    });
    assert.notEqual(hashFactFingerprint([withoutGaps]), hashFactFingerprint([withGaps]));
  });
