...
describe("runCampSource", () => {
  ...
  it("AAA → BBB → BBB: run 3 compares against BBB, not AAA", async () => {
    ...
    const store = createMemoryIngestionStore({ sources: [seed] });
    const newId = createSequentialIdFactory("row");
    const rawAaa = campPage("$399");
    const rawBbb = campPage("$399").replace(
      "<body>",
      '<body><!-- raw-bbb --><div class="chrome" data-build="bbb"></div>',
    );
    ...
    const run = (content: string, now: string) =>
      runCampSource({
        ...
        store,
        fetcher: new FixtureSourceFetcher(
          { [seed.id]: { content, fetchStatus: "success" } },
          { newId },
        ),
        catalog: catalog(),
        now: () => new Date(now),
    ...
    const first = await run(rawAaa, "2026-09-12T12:00:00.000Z");
    assert.equal(first.status, "baseline");
    const afterFirst = store.snapshotState();
    ...
    assert.equal(afterFirst.sources[0].lastContentHash, hashAaa);
    assert.ok(fingerprintXyz);
    assert.equal(changedAtBaseline, "2026-09-12T12:00:00.000Z");

    const second = await run(rawBbb, "2026-09-13T12:00:00.000Z");
    ...
    assert.equal(afterSecond.candidates.length, candidatesAfterFirst);
    assert.equal(afterSecond.reviewDecisions.length, reviewsAfterFirst);

    const third = await run(rawBbb, "2026-09-14T12:00:00.000Z");
    assert.equal(third.status, "unchanged_raw");
    assert.equal(third.extractionRunId, null);
    assert.equal(third.candidateIds.length, 0);
    ...
    assert.equal(afterThird.reviewDecisions.length, reviewsAfterFirst);
    assert.equal(afterThird.sources[0].lastContentHash, hashBbb);
    assert.equal(afterThird.sources[0].lastFactFingerprint, fingerprintXyz);
    assert.equal(afterThird.sources[0].lastChangedAt, changedAtBaseline);
    // If run 3 still compared against AAA, BBB would look new and extraction would run.
