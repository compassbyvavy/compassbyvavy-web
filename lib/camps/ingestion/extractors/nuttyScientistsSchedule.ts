 ...
 * `startDate`/`endDate` null. It never invents a year. Month/day splitting is
 * the shared `parseMonthDayWindows` helper; Nutty-only OCR repair stays here.
 *
...
export type NuttyWeekWindow = MonthDayWindow;
...
export function nuttyWeekIdentity(
  ...
  return monthDayWindowIdentity(startMonth, startDay, endMonth, endDay);
}
...
export function parseNuttyWeekWindows(rawInput: string): NuttyWeekWindow[] {
  return parseMonthDayWindows(repairNuttySourceText(rawInput));
}
