...
describe("playbook features that stay unsupported until verified data exists", () => {
  ...
  it("program grouping mode uses the later grouping label without dropping sibling overlap", () => {
    const programOption = LISTING_VIEW_OPTIONS.find((option) => option.id === "program");
    assert.equal(programOption?.label, "Group sessions by camp");
  });
});
