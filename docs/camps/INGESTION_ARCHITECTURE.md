...
### knownGaps versioning

`knownGaps` are deterministic negative/absence signals derived by the extractor
(for example `own_counsellor_background_screening_policy_not_stated`). They stay
on the program fingerprint allowlist.

If known-gap **detection semantics** change because extraction capability
changed, bump `FACT_FINGERPRINT_VERSION` (`camp-facts-v1` → the next version)
or explicitly rebaseline. Do not let “extractor got better” be interpreted as
“the provider changed facts.” Keep `knownGaps` fingerprinted; do not drop them
from the payload.
...
- **Known-gap semantics.** Facts the page does not state (own-counsellor
  ...
  gaps. Absence is not false. Those gaps are fingerprinted; a later extractor
  that becomes able to read a previously-unstated policy must bump
  `FACT_FINGERPRINT_VERSION` or rebaseline rather than silently rewriting facts.
